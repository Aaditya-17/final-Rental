...
const HeroSection = () => {
  ...
  return (
    <section className="relative flex flex-col items-center justify-center h-screen">
      ...
      <div className="absolute top-55 right-55 transform -translate-y-1/2 w-[50%] text-right">
        <motion.h1
          className="text-3xl sm:text-4xl font-bold text-green-100 dark:text-gray-800"
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.8 }}
        >
          {t("smart_farming")}{" "}
          <span className="text-green-700">{t("simplified_rentals")}</span>
        </motion.h1>
        <motion.p
          className="mt-3 text-lg text-green-100 dark:text-gray-800 max-w-lg ml-auto"
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        {/* Buttons */}
        <motion.div
          className="mt-5 flex flex-col sm:flex-row sm:space-x-4 space-y-3 sm:space-y-0 justify-end"
          initial={{ opacity: 0, x: 50 }}
          animate={{ opacity: 1, x: 0 }}
        ...
        >
          <Link
            to="/bookequipment"
            className="bg-green-700 hover:bg-green-700 text-white px-6 py-3 rounded-lg transition-all duration-300 transform hover:scale-105"
          >
            {t("browse_equipment")}
          </Link>
          <Link
            to="/signup"
            className="bg-gray-100 dark:bg-gray-800 dark:text-white text-green-700 px-6 py-3 rounded-lg transition-all duration-400 transform hover:scale-105"
          >
            {t("get_started")}
          </Link>
        </motion.div>
